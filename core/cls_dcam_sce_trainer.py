...
class ClsDcamSceTrainer:
    def __init__(self, model, dcam_sce_loss_weight, device, optimizer, data_loaders, dataset_sizes, num_epochs,
                 result_path, model_path):
        self.model = model
        self.dcam_sce_loss_weight = dcam_sce_loss_weight
        self.device = device
            ...
            for phase in ['train']:
                ...
                for i, samples in enumerate(self.data_loaders[phase]):
                    ...
                    with torch.set_grad_enabled(True):
                        _, loss_ce, acc = self.model(inputs, labels)
                        loss_ce = torch.mean(loss_ce)
                        acc = torch.mean(acc)
                        loss = self.dcam_sce_loss_weight * loss_ce

                        print('\r[{}/{}] loss_cls:{:.4f}'.format(i, len(
                            self.data_loaders[phase]), loss_ce), end='', flush=True)
                    ...
                    running_loss_cls += loss_ce.item() * inputs.size(0)
    ...
    def check(self):
        ...
        for i, samples in enumerate(self.data_loaders[phase]):
            ...
            with torch.no_grad():
                ...
                running_loss += loss_ce.item() * inputs.size(0)
                for j in range(len(labels)):
                    running_corrects += acc
