...
class ClsTrainer:
    def __init__(self, model, criterion, optimizer, data_loaders, dataset_sizes, device, num_epochs, result_path,
                 model_path, batchsize):
        self.model = model
            ...
            for phase in ['train']:
                ...
                for i, samples in enumerate(self.data_loaders[phase]):
                    ...
                    with torch.set_grad_enabled(True):
                        ...
                        print('\r[{}/{}] loss:{:.4f}'.format(i, len(self.data_loaders[phase]), loss), end='',
                              flush=True)
...
class TrainerHistory(object):
    ...
    def update(self, phase, loss, acc, istrain=False):
        if phase == 'train':
