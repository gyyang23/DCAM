...
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            outputs = [model.forward(img[0].cuda(non_blocking=True)) for img in pack['img']]  # b x 20 x w x h
...
def run(args):
    ...
    model.load_state_dict(torch.load(osp.join(args.dcam_weight_dir, 'checkpoint.pth')), strict=True)
    model.eval()
